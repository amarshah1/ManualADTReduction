...
# for "function", the second element is the name of the function, "Cons", "Nil", "is_Cons", "is_Nil", "Head", "Tail" is the functions name
    ...
    for stmt in query:
        ...
        if stmt[0] == "and" or stmt[0] == "or":
            ...
            if rhs[1] == "Cons":
                ...
                inner_clause.append(stmt)
                inner_clause.append(("or", [("and", first_clause, []), second_clause]))
            elif rhs[1] == "is_Cons":
        ...
        return_clause.append(("and", inner_clause, []))
...
def reduction_with_axioms(query, variables):
    ...
    for var in keys:
        if variables[var] == "MyList":
            axioms.append(("or", [("function", "is_Cons", [var]), ("function", "is_Nil", [var])], []))
            axioms.append(("or", [("not", ("function", "is_Cons", [var]), []), ("not", ("function", "is_Nil", [var]), [])], []))
...
def query_list_to_smt(query_list, variables = {}):
...
(declare-fun is_Cons (MyList) Bool) \n"""
    ...
    for query in query_list:
        string += "(assert " + query_to_smt(query) + " )\n"
