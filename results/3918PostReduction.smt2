...
(assert (and (and (and (is-Cons l1) (= l1 (Cons cv0 cv1)) (= cv0 (Head l1)) (= cv1 (Tail l1))) (and (= l1 Nil) (is-Nil l1)) (and (= l2 Nil) (is-Nil l2)) (and (is-Nil Nil) (= Nil Nil)))) )
(assert (and (not (is-Cons Nil))) )
(assert (and (or (and (is-Cons l3) (= l3 (Cons cv2 cv3)) (= cv2 (Head l3)) (= cv3 (Tail l3))) (and (is-Cons Nil) (= Nil (Cons cv4 cv5)) (= cv4 (Head Nil)) (= cv5 (Tail Nil))) (and (is-Nil l1) (= l1 Nil)) (and (= l0 l0)))) )
(assert (and (and (and (= l2 Nil) (is-Nil l2)) (and (is-Cons l2) (= l2 (Cons cv6 cv7)) (= cv6 (Head l2)) (= cv7 (Tail l2))) (and (= l3 Nil) (is-Nil l3)) (and (is-Cons Nil) (= Nil (Cons cv8 cv9)) (= cv8 (Head Nil)) (= cv9 (Tail Nil))))) )
(assert (and (or (and (is-Nil Nil) (= Nil Nil)) (and (= l1 Nil) (is-Nil l1)) (and (= l1 l3)) (and (is-Cons Nil) (= Nil (Cons cv10 cv11)) (= cv10 (Head Nil)) (= cv11 (Tail Nil))))) )
