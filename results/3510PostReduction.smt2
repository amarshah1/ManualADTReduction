...
(assert (and (or (and (is-Nil Nil) (= Nil Nil)) (and (is-Cons l1) (= l1 (Cons cv0 cv1)) (= cv0 (Head l1)) (= cv1 (Tail l1))) (and (= l1 l3)))) )
(assert (and (is-Cons Nil) (= Nil (Cons cv2 cv3)) (= cv2 (Head Nil)) (= cv3 (Tail Nil))) )
(assert (and (and (and (= l2 l0)) (and (is-Cons l3) (= l3 (Cons cv4 cv5)) (= cv4 (Head l3)) (= cv5 (Tail l3))) (and (is-Nil Nil) (= Nil Nil)))) )
(assert (and (or (and (= l1 Nil) (is-Nil l1)) (and (= l2 Nil) (is-Nil l2)))) )
(assert (and (= l2 l2)) )
(assert (and (is-Cons Nil) (= Nil (Cons cv6 cv7)) (= cv6 (Head Nil)) (= cv7 (Tail Nil))) )
(assert (and (and (and (is-Cons l1) (= l1 (Cons cv8 cv9)) (= cv8 (Head l1)) (= cv9 (Tail l1))) (and (= l3 l2)))) )
(assert (and (= l2 l2)) )
